...
- 旧アプリのデータ構造は、このアプリの要件に対してより適していると判断しました。
- そのため、**データ構造は旧アプリの考え方へ戻す**方針とします。
- 変更点は、`E7payment` を **口座別に扱えるようにしたこと** が中心です。
- 今後は、永続データ構造を安易に変更しません。
- 画面仕様の都合で複雑さが必要になった場合も、まずはサービス層や表示側で吸収し、永続モデルの変更は慎重に行います。
...
### `paid / unpaid` を `Bool` から所属表現へ戻した

- `E2invoice.isPaid` / `E7payment.isPaid` の保存値はやめました。
- 状態は所属リレーションで持ち、`isPaid` は計算プロパティに戻しました。
- これにより、旧アプリの考え方と一覧表示性能を合わせやすくしました。
...
- 今回の設計見直しでは、旧構造の方が要件に合うと判断して戻しています。
...
- 未払/済み切替では `payment` 自体が別オブジェクトへ移ることがあります。
- そのため Undo は `payment` 参照ではなく、**対象 `invoice` 配列**を保持して戻す方が安全です。

### 今後の変更方針

- 永続データ構造の変更は、原則として行いません。
